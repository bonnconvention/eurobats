...
$is_res_block = ( db_query( "SELECT COUNT( field_doc_form_value )
                ...
                WHERE ( resac.entity_id = $nid ) AND ( field_doc_form_value = 'Resolution' )" )->fetchField() != "0" );

$mdoc_title = "Meeting&nbsp;Documents";
$mres_title = "Resolutions/Annexes";
$mpic_title = "Pictures";
...
$fldlist1 = "field_doc_form_value AS rubric, uri, field_doc_file_mop_description AS description, field_doc_id_mop_value AS docid";
$fldlist2 = "COUNT( * )";
...
$sample1 = "<table style=\" margin-left : 20px; padding-bottom : 15px; \"><td>@C0@</td><td style=\"padding-left : 60px;\">@C1@</td><td style=\"padding-left : 60px;\">@C2@</td><td style=\"padding-left : 60px;\">@C3@</td></table>";
$fldsamples = Array( "<a href=\"@HREF@\" target=\"_blank\">Record of the Meeting</a>",
    "<a href=\"$node_url#3\">$mres_title</a>",
...
<?php 
        ...
        // Pictures
        if ( $is_pic_block ) {
            $out = str_replace( "@C$i@", str_replace( "@HREF@", file_create_url( $s1 ), $fldsamples[ 3 ] ), $out );
            $i++;
        }
...
<?php if ( $is_res_block ) { ?>
    <li id="res_swli">
        <a id="res_swa" href="javascript:void( 0 )" onclick="switchBlock( 2 )"><?php echo $mres_title; ?></a>
    </li>
<?php } ?>
<?php if ( $is_pic_block ) { ?>
    <li id="pic_swli">
...
foreach ( $res as $row ) {
    ...
    if ( $row->rubric == "Resolution" ) {
        $out1 .= str_replace( "@FNAME@", $fname, str_replace( "@DESC@", $row->description, str_replace( "@HREF@", file_create_url( $row->uri ), $sample5 ) ) );
    }
    else {
...
if ( $is_res_block ) {
?>
<div id="res_block" style="display : none">
<?php echo "<table>" . $out1 . "</table>"; ?>
</div>
<?php }//! ---------------------------------- pic block ----------------------------------  
if ( $is_pic_block || $is_res_block ) { ?>
...
var switch_objs =
    [
        ...
        [ document.getElementById( "res_swa" ), document.getElementById( "res_swli" ), document.getElementById( "res_block" ) ]
    ];

function switchBlock( val )
{
    for ( i = 0; i < switch_objs.length; i++ ) {
        if ( i == val ) {
            cName = "active";
            disp = "block";
        }
        ...
        if ( switch_objs[ i ][ 0 ] != null ) {
            switch_objs[ i ][ 0 ].className = cName;
            switch_objs[ i ][ 1 ].className = cName;
            switch_objs[ i ][ 2 ].style.display = disp;
        }
    }
