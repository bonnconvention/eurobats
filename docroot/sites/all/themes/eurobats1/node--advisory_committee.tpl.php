...
$fldlist1 = "field_doc_form_value AS rubric, uri, field_doc_file_ac_description AS description, field_doc_id_ac_value AS docid";
$fldlist2 = "COUNT( * )";
$sqlsample = "SELECT @FLDLST@
                FROM field_data_field_resources_ac AS resac INNER JOIN field_data_field_doc_file_ac AS docf ON field_resources_ac_value = docf.entity_id
                    LEFT JOIN field_data_field_doc_form AS fsdoctype ON docf.entity_id = fsdoctype.entity_id
                    LEFT JOIN file_managed ON field_doc_file_ac_fid = file_managed.fid
                    LEFT JOIN field_data_field_doc_id_ac AS docid ON docf.entity_id = docid.entity_id
