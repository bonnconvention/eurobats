...
jQuery(document).ready(function($) {
	...
	if(!Drupal.settings.eurobats2.blockdemo){
		$('#topBar').hide();
	}
	
	// set the right text in the blcok demo page
	if(Drupal.settings.eurobats2.blockdemo){
		$('#topBarLink a').text("↑ " + Drupal.settings.eurobats2.bartext2 + "");
	}
	
	// Make the spy effect
	$('#topBarLink a').live('mouseover mouseout', function(event) {
    if (event.type == 'mouseover' && $(this).text() == "↓ " + Drupal.settings.eurobats2.bartext + "") {
      $('#topBarLink').animate({
        'padding-top':'5px'
      });
    }
    else if (event.type == 'mouseout' && $(this).text() == "↓ " + Drupal.settings.eurobats2.bartext + "") {
    $('#topBarLink').animate({
      'padding-top':'0px'
    });
    }
    ...
    if ($(this).text()=="↓ " + Drupal.settings.eurobats2.bartext + "") {
      $(this).text("↑ " + Drupal.settings.eurobats2.bartext2 + "");
      $(this).attr('title','Close this region');
    }
    else if ($(this).text()=="↑ " + Drupal.settings.eurobats2.bartext2 + "") {
      $(this).text("↓ " + Drupal.settings.eurobats2.bartext + "");
