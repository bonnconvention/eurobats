 ...
 */
function eurobats2_preprocess_page(&$vars) {
  // Useful for devel default banners, remove before commit
  //   variable_del('theme_eurobats2_first_install');
  // Chcek if is first setup of eurobats2 and install banners.
  if (variable_get('theme_eurobats2_first_install', TRUE)) {
  ...
  $vars['border']       = TRUE;
  $vars['layout_width'] = eurobats2_page_width(theme_get_setting('layout_width'));
  ...
  $vars['topbarlink'] = l('&darr; ' . t(check_plain(theme_get_setting('bartext'))), current_path(),
    array(
      'attributes' => array(
        ...
        'class' => array('openregion eurobats2-hide-no-js'),
      ),
      'html' => TRUE
    )
  );
  ...
  // BANNER SECTION ============================================================
  // Banner to display
  $banners = eurobats2_show_banners();

  // Banners section
  $vars['banner_image'] = eurobats2_banners_markup($banners);
  $vars['banner_text'] = '';
  $vars['banner_nav'] = '';
 ...
 */
function eurobats2_preprocess_node(&$vars){
 ...
 */
function eurobats2_preprocess_block(&$vars){ // title visibility
  $vars['blockhide'] = "";
 ...
 */
function eurobats2_preprocess_comment(&$vars) {
...
function eurobats2_preprocess_html(&$vars){
 ...
 * Breadcrumb.
 */
function eurobats2_breadcrumb($variables) {
 ...
 */
function eurobats2_get_banners($all = TRUE) {
  // Get all banners
  $banners = variable_get('theme_eurobats2_banner_settings', array());
 ...
 *    Settings to save
 */
function eurobats2_set_banners($value) {
  variable_set('theme_eurobats2_banner_settings', $value);
}
